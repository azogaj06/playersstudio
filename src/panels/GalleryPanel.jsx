...
function Lightbox({ index, onClose, onMove }) {
  ...
  // panel underneath (Panel skips defaultPrevented events). Tab is trapped
  // inside the dialog.
  useEffect(() => {
    const onKey = (e) => {
      if (e.key === 'Escape') {
  ...
  return (
    <div
      ...
      onClick={onClose}
      onTouchStart={(e) => {
        // A second finger means pinch-zoom, not navigation.
        touchX.current = e.touches.length === 1 ? e.touches[0].clientX : null
      }}
      onTouchEnd={(e) => {
        if (touchX.current == null) return
        if (e.touches.length > 0) return // fingers still down — not a swipe
